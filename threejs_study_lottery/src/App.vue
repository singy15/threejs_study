...
function initWorld() {
  ...
  const spawnStaticBox = (scene, world, pom, sizeX, sizeY, sizeZ) => {
    let po = new PhysicsObject();
    //po.timer = 2000;
    po.scene = scene;
    po.world = world;
    (() => {
      ...
      let mesh;
      (() => {
        const geometry = new THREE.BoxGeometry(sizeX * 2, sizeY * 2, sizeZ * 2);
        const material = new THREE.MeshPhysicalMaterial(
          /*MeshStandardMaterial*/ {
            color: 0xffffff, // randomRange(0x000000, 0xffffff),
            roughness: 0.2,
            //wireframe: true,
            //transmission: 0.9,
          },
  ...
  boxBottom.body.position.y = y - boxSize / 2;
  let boxLeft = spawnStaticBox(scene, world, pom, 1, boxSize / 2, boxSize / 2);
  boxLeft.body.position.y = y;
  boxLeft.body.position.x = x - boxSize / 2;
